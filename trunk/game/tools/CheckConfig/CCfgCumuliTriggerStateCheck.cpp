...
CCfgCumuliTriggerStateCheck::SetRowName CCfgCumuliTriggerStateCheck::ms_setRowName;

bool CCfgCumuliTriggerStateCheck::Check(const TCHAR* cfgFile)
{
	using namespace CfgChk;

	CTxtTableFile TxtTableFile;
	SetTabFile(TxtTableFile, "积累触发状态");
	...
	static const uint32 s_uTableFileWide = 17;
	...
	for(int32 i = 1; i < TxtTableFile.GetHeight(); ++i)
	{
		...
		ReadItem(strTriggerEff, szTriggerState_TriggerEff);
...
void CCfgCumuliTriggerStateCheck::EndCheck()
{
	ms_setRowName.clear();
}

bool CCfgCumuliTriggerStateCheck::CheckExist(const string& strName)
{
	SetRowName::iterator iter = ms_setRowName.find(strName);
	if (iter == ms_setRowName.end())
	{
		stringstream ExpStr;
		ExpStr << " 违反魔法操作约束: 积累触发状态" << " [" << strName << "] 不存在 ";
