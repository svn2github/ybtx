...
public:
	...
	void					ShowIntNpcName(vector<string>& characterNames);
	void					ShowIntObjName(vector<string>& objectNames);
	void					DisPlayCharacter(string name, CRenderObject* renderObj,EModelType type);
	void					GetAnimateNames (CRenderObject* randerObj, vector<string>& agpNames);
...
inline string CDataCache::GetModelFileName()  const
{
	return m_curModelFileName;
}
