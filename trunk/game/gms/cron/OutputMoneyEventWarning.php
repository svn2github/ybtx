...
function MainFun($server_id,$time_step,$max_money,$max_times){
	...
	while($row = mysql_fetch_row($result)){
		$rs = mysql_query("insert into tbl_output_money_warning values('$max_dt','$server_id','$row[0]','$row[1]','$row[2]','$row[3]')");
		if(!$rs) echo mysql_error();
		printf("报警时间：%s\t角色ID：%s\t金钱来源：%s\t产出金额：%d\t产出次数：%d\n",$max_dt,$row[0],$row[1],$row[2],$row[3]);
	}
	mysql_close($Conn);
	return true;
}
