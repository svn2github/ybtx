...
/* This class implements a simple cache. A cache object is instantiated
   ...
   object derived from CDSBaseObject (helps reduce memory leaks). The cache
...
/* Making these classes inherit from CDSBaseObject does nothing for their
...
private:
    ...
    /* These are initialised in the constructor. The first variable points to
       an array of pointers, each of which points to a CDSBaseObject derived
...
public:
    ...
    CDSBaseObject *AddToCache(CDSBaseObject *pObject);

    /* Remove an item from the cache */
    CDSBaseObject *RemoveFromCache();
