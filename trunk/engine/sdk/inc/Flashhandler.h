...
class FlashHandler : public ShockwaveFlashObjects::_IShockwaveFlashEvents  //执行flash命令和执行AS脚本
{
public:	
...
public:
	...
	HRESULT Init(CFlashPlayer* owner)
	{
		this->owner = owner;
		this->owner->comCount++;
		...
		if ((owner->m_shwaveflash->QueryInterface(IID_IConnectionPointContainer, (void**)&cPointContainer) == S_OK) &&
	...
	{
		owner->HandleFlashCall((wchar_t*)request);  //回调C++函数
		return S_OK;
	}
};
