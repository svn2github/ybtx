...
template<class T> struct TVector3 : public CMathMallocObject
{
	...
	// 用矢量[x,y,z,0]和矩阵相乘，用于法线操作
	template<class MATRIX> const TVector3& MultiplyNormal( const MATRIX& mat);
	// 到线段最短距离
...
template<class T> const TVector3<T> TVector3<T>::ZERO( T(0), T(0), T(0) );
template<class T> const TVector3<T> TVector3<T>::UNIT_X( T(1), T(0), T(0) );
template<class T> const TVector3<T> TVector3<T>::UNIT_Y( T(0), T(1), T(0) );
template<class T> const TVector3<T> TVector3<T>::UNIT_Z( T(0), T(0), T(1) );
template<class T> const TVector3<T> TVector3<T>::UNIT_SCALE( T(1), T(1), T(1) );
...
// [x,y,z,0]和矩阵相乘，用于发现操作
template<class T> template<class MATRIX> 
inline const TVector3<T>& TVector3<T>::MultiplyNormal( const MATRIX& Mat )
{
	T fX = x*Mat._11 + y*Mat._21 + z*Mat._31;
	T fY = x*Mat._12 + y*Mat._22 + z*Mat._32;
	T fZ = x*Mat._13 + y*Mat._23 + z*Mat._33;
...
template<class T> inline TVector3<T>::operator T* (void)
{
	return (T*)v;
}

template<class T> inline TVector3<T>::operator const T* (void)
{
	return (T*)v;
}
