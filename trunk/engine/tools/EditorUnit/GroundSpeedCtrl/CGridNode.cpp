...
void CGridNode::RenderDebug()
{
	m_uPreTime				= m_uCurTime;
	m_uCurTime				= (uint32)GetProcessTime();
	uint32 deltaTime = m_uCurTime - m_uPreTime;
	m_MoveNow += deltaTime * m_fGroundSpeed * 0.064f;
	m_MoveNow = fmod( m_MoveNow , 64.0f );
	float	DeltaMove = fmod( m_MoveNow , 64.0f );
	...
	for ( uint32 w = 0; w<m_uRange * 2; ++w )
	{
		for( uint32 h = 0; h<m_uRange * 2; ++h)
		{
			...
			if( w == 0 )
			{
				pLineBuf[lsize + 0].p = CVector3f( float((tempPos.x)*eGridSpan),	pLogicData->GetLogicHight( tempPos.x - 1, tempPos.y-1 )	+ 1.0f,	float(tempPos.y		*eGridSpan) - DeltaMove);
				pLineBuf[lsize + 1].p = CVector3f( float((tempPos.x)*eGridSpan),	pLogicData->GetLogicHight( tempPos.x - 1, tempPos.y   ) + 1.0f,	float((tempPos.y+1)	*eGridSpan) - DeltaMove);
			...
			{
				pLineBuf[lsize + 0].p = CVector3f( float(tempPos.x    *eGridSpan),	pLogicData->GetLogicHight( tempPos.x - 1, tempPos.y -1 ) + 1.0f, float((tempPos.y)	*eGridSpan) - DeltaMove);
				pLineBuf[lsize + 1].p = CVector3f( float((tempPos.x+1)*eGridSpan),	pLogicData->GetLogicHight( tempPos.x	, tempPos.y -1 ) + 1.0f, float((tempPos.y)	*eGridSpan) - DeltaMove);	
				pLineBuf[lsize + 0].diffuse = pLineBuf[lsize + 1].diffuse =	tTickLineColor;
				lsize += 2;
			}

			pLineBuf[lsize + 0].p = CVector3f( float(tempPos.x    *eGridSpan)				,	0.0f,	float((tempPos.y+1)	*eGridSpan) - UINT_SIZE - DeltaMove);
			pLineBuf[lsize + 1].p = CVector3f( float((tempPos.x+1)*eGridSpan)				,	0.0f,	float((tempPos.y+1)	*eGridSpan)	- UINT_SIZE - DeltaMove);
			pLineBuf[lsize + 2].p = CVector3f( float(tempPos.x    *eGridSpan) + UINT_SIZE	,	0.0f,	float(tempPos.y		*eGridSpan) - DeltaMove);
			pLineBuf[lsize + 3].p = CVector3f( float(tempPos.x    *eGridSpan) + UINT_SIZE	,	0.0f,	float((tempPos.y+1)	*eGridSpan) - DeltaMove);
			...
			pLineBuf[lsize + 4].p = CVector3f( float(tempPos.x    *eGridSpan),	pLogicData->GetLogicHight( tempPos.x - 1		, tempPos.y   ) + 1.0f,	float((tempPos.y+1)	*eGridSpan) - DeltaMove);
			pLineBuf[lsize + 5].p = CVector3f( float((tempPos.x+1)*eGridSpan),	pLogicData->GetLogicHight( tempPos.x			, tempPos.y   ) + 1.0f,	float((tempPos.y+1)	*eGridSpan) - DeltaMove);	
			pLineBuf[lsize + 6].p = CVector3f( float((tempPos.x+1)*eGridSpan),	pLogicData->GetLogicHight( tempPos.x			, tempPos.y-1 )	+ 1.0f,	float(tempPos.y		*eGridSpan) - DeltaMove);
			pLineBuf[lsize + 7].p = CVector3f( float((tempPos.x+1)*eGridSpan),	pLogicData->GetLogicHight( tempPos.x			, tempPos.y   ) + 1.0f,	float((tempPos.y+1)	*eGridSpan) - DeltaMove);
...
float CGridNode::GetGroundSpeed( void )
{
	return m_fGroundSpeed;
}

void CGridNode::SetGroundSpeed( float speed )
{
	m_fGroundSpeed = speed;
}

CGridNode::CGridNode( ERenderObjStyle style , CRenderScene* pScene ) 
