...
void CEditUnitPropHelp::SetBaseUintData()
{
	
	CCombDataHelper* pCombHelper = (CCombDataHelper*)_RegCtrlData(E_COMBO, NULL, NULL, "连接点", NULL, &(m_pEffectUnitProp->m_eUpdatePos));
	pCombHelper->PushItem("固定", UP_FIXED);
	pCombHelper->PushItem("跟随Socket偏移", UP_SOCKETOFFSET);
	pCombHelper->PushItem("跟随骨骼偏移", UP_NODEOFFSET);
	pCombHelper->PushItem("跟随Socket", UP_SOCKET);
	pCombHelper->PushItem("跟随骨骼", UP_NODE);

	_RegCtrlData(E_ESTRING, NULL, NULL, "连接对象名字", NULL, &(m_pEffectUnitProp->m_szAttachName) );
	_RegCtrlData(E_ESETESTRING, NULL, NULL, "关联的动画", NULL, &(m_pEffectUnitProp->m_szAniName) );
	_RegCtrlData(E_ESTRING, NULL, NULL, "自定义类名", NULL, &(m_pEffectUnitProp->m_EffectClassName) );
	_RegCtrlData(E_INTEGER, NULL, NULL, "贴图重复次数", NULL, &(m_pEffectUnitProp->m_nTextureCircle) );
	_RegCtrlData(E_BOOLValue, NULL, NULL, "缓慢消失", NULL, &(m_pEffectUnitProp->m_bSlownDisappear) );
}
...
void CEditUnitPropHelp::SetUnitVisible(bool isShow)
{
	m_isUnitVisible = isShow;
}

bool  CEditUnitPropHelp::GetUnitVisible(void)
{
	return m_isUnitVisible;
}