...
<br />
<p><b>所有服务器端的log如下</b></p>
<table border=1 style='table-layout:fixed;wrap'>
<?php
...
function ViewLog()
{
	...
	$nf = intval($_GET["nf"]);
	if ($nf == 1)
		$query_by_nf = " and slc.network_flag = 1";
	else if ($nf == 2)
		$query_by_nf = " and slc.network_flag = 2";
	...
	$qs = "select count(distinct slc.error_code) from tbl_server_log_common slc, tbl_log_info li, tbl_guid_info gi where li.error_code = slc.error_code and gi.guid = slc.server_guid and li.error_type='程序' $query_by_user $query_by_nf $query_by_version"; 
	...
	$qs = "select li.error_code, li.error_title, max(slc.happened_time) as last, min(slc.happened_time) as first, count(slc.happened_time), li.error_owner, li.error_status, max(slc.version)  $server_from_info $server_where_info and li.error_type='程序' $query_by_user $query_by_nf $query_by_version group by li.error_code $order_str limit $b, $e";
	$result = mysql_query($qs);
	
	$str = sprintf("<tr bgcolor='#9acd32'>\n")
		.sprintf("<th  width='400px'>log标题(点击查看堆栈)</th><th width=100>错误码</th><th width=125><a href='list_serv_log.php?order=1&page=1&who=$who&nf=$nf&version=$version'>最后发生时间</a></th><th width=125><a href='list_serv_log.php?order=2&page=1&who=$who&nf=$nf&version=$version'>首次发生时间</a></th><th width=75>次数(点击查看列表)</th><th width='100'>"
		.GetNetFlagTitle($nf, "list_serv_log.php?order=$order&page=1&who=$who&version=$version")
		."</th><th width=75>")
		.GetOwner($who, "list_serv_log.php?order=$order&page=1&nf=$nf&version=$version")
		.sprintf("</th><th width=75>处理状态</th><th width=75>")
		.GetVersion("s", $version, "list_serv_log.php?order=$order&page=1&nf=$nf&who=$who")
		.sprintf("</th>\n")
		.sprintf("</tr>\n");

	$str_code = "<a href='list_by_code_s.php?page=1&code=%s' target='_blank'>%s</a>";
	...
	while($row=mysql_fetch_row($result))
	{
		...
		$str = $str.sprintf("<tr bgcolor='%s'>\n", $color)
			.sprintf("<td align='left'><div id='wrap'>&nbsp;").sprintf($str_title, urlencode($code), str_replace('%', '%%', $title)).sprintf("</div></td>\n")
...
<!--
<?php
	printf("var url='list_serv_log.php?order=%d&who=%s&nf=%d&version=%s&';\n", $order, $who, $nf, $version);
