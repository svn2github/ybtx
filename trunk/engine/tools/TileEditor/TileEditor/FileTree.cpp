...
void CFileList::PreSubclassWindow()
{
...
BOOL CFileList::SetFileIcon(WORD ID,LPCTSTR extname,BYTE pos)
{
	sprintf(m_FileExt[pos].ExtName,extname);
	HICON ico=LoadIcon(AfxGetInstanceHandle(),(LPCTSTR)ID);
	m_FileExt[pos].ImagNum=m_Image.Add(ico);
	return 1;
}
...
BOOL CFileList::ResetList(LPCTSTR path)
{
	...
	for(int i=0;i<256;i++)
	{
		...
		if(buf[n-1]=='\\'||buf[n-1]=='/')
			buf[n-1]=0;
		strcat(buf,"\\*.");
		strcat(buf,m_FileExt[i].ExtName);
		...
		if(h!=INVALID_HANDLE_VALUE)
		{
			...
			while(re)
			{
				...
				lvIt.iImage=m_FileExt[i].ImagNum;
...
BOOL CFileTree::InitChild(FLODER& floder,int order)
{
	...
	while(elist->Next(1,&plist,&num)==S_OK)
	{
		...
		if(dis.uType==STRRET_CSTR)
			...
			diss=CString(((char*)plist)+dis.uOffset);
		...
		if(path.uType==STRRET_CSTR)
			...
			paths=CString(((char*)plist)+path.uOffset);
...
BEGIN_MESSAGE_MAP(CDirSelectWnd, CWnd)
	//{{AFX_MSG_MAP(CDirSelectWnd)
		// No message handlers
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

LRESULT CDirSelectWnd::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{		
	if( message== WM_CLOSE )
		PostQuitMessage(0);
...
CString CDirSelectWnd::GetDirector( const CString InitDir, CString Title )
{
    ...
    CString className=AfxRegisterWndClass( CS_DBLCLKS, LoadCursor(NULL,IDC_ARROW), (HBRUSH)(COLOR_MENU+1), NULL );
	CWnd::CreateEx( 0, className, Title, WS_SYSMENU|WS_OVERLAPPED, CRect( 0,0,275,330 ),NULL,NULL );

	m_Tree.Create( this );
	SetWindowLong( m_Tree.m_hWnd, GWL_EXSTYLE, WS_EX_CLIENTEDGE );
	m_Tree.SetWindowPos( NULL, 10, 10, 250, 250, SWP_SHOWWINDOW );
	m_Tree.SetCurPath( InitDir );

	CFont Font;
	Font.CreateFont( 12, 0, 0, 0, 0, 0, 0, 0, DEFAULT_CHARSET, 0, 0, PROOF_QUALITY, 0, "" );
	m_OK.Create( "确定", WS_CHILD, CRect( 185, 270, 255, 292 ), this, IDOK );
	m_Cancel.Create( "取消", WS_CHILD, CRect( 100, 270, 170, 292 ), this, IDCANCEL );
