...
#include "BluelyGUI.h"
#include "Input.h"
#include "../TileEditorEngine/memfile.h"
...
void CEngine::Destroy()
{
	...
	CSceneManager::GetInstance()->Drop();
}
...
void CEngine::Update()
{
	...
	CSceneManager * pScene = CSceneManager::GetInstance();
...
void CEngine::Render()
{
	...
	if( ::GetTickCount() - LastUpdateTime > 32)
	{
		...
		bly::CTileEditApp::GetInstance()->GetOperator()->GetCoreGraphic()->RenderEndForMapEditor(false);
		pRender->GetRawDevice()->SetRenderState( D3DRS_AMBIENT, 0xffffffff);
...
CCamera * CEngine::GetActiveCamera()
{
	return m_pCamera;
}
