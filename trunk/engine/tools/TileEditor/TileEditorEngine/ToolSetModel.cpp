...
void CToolSetModel::CreateActiveObject(std::vector<std::string> & veCTileEditorModelFileNames, std::vector< std::vector<CTileEditorModel::SBlockInfo> > & vvBlock, std::vector< std::vector<CTileEditorModel::SLogicHeightInfo> > & vvHeight, bool bSubUnit, CTileEditorModel::ECenter_Type eBlockType, float fLift, float fShiftX, float fShiftZ)
{
	...
	std::vector<CTileEditorModel*> vec;
	...
	{
		CTileEditorModel * p = CSceneManager::GetInstance()->AddNodeModel(*iter, bSubUnit, fLift, fShiftX, fShiftZ, BlockInfoNum, LogicHeightInfoNum);
		vec.push_back(p);

		vvBlock[i].resize(BlockInfoNum);
		vvHeight[i].resize(LogicHeightInfoNum);
		//if ( vvBlock[i].size() != 0 )
		//{
