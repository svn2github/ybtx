...
struct LTVector
{
	...
	LTVector(){}
	LTVector(float fx, float fy, float fz){ x = fx; y = fy; z = fz;}

	void Init(){x = 0; y = 0; z = 0;}
	void Init(float fx, float fy, float fz){ x = fx; y = fy; z = fz;}
};
...
struct GenericProp 
{
	...
	{
		sprintf(m_String, "%f %f %f", vValue.x, vValue.y, vValue.z);
	}
...
public:
	...
	bool	ReserveProps(uint32 nMax, bool bPreserveProps)	
	{
		...
		//preserving)
		uint32 nNumToCopy = m_nNumProps<nMax?m_nNumProps:nMax;
	...
	bool	AddProp(const char* pszName, const GenericProp& Prop)
	{
		...
		uint32 nPropLocation = nUpper>nLower?nUpper:nLower;
