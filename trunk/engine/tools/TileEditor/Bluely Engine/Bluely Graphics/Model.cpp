...
CModel::CModel(ISceneNode * pParent) : ISceneNode(pParent)
{
	m_pModel = NULL;
...
void CModel::SetCenterTypeOrigin(ECenter_Type eCenterType)
{
	m_eCenterType = m_eCenterTypeOrigin = eCenterType;
}

CModel::ECenter_Type CModel::GetCenterType()
{
	return m_eCenterType;    
}

void CModel::SetBlockTypeByIndex(int nIndex, int nBlockType)
{
	m_vecBlockInfo[nIndex].nBlockType = nBlockType;
}

CModel::ECenter_Type CModel::GetCenterTypeOrigin()
{
	return m_eCenterTypeOrigin;    
}

void CModel::Create(string strFileName, bool bSubUnit, float fLift, float fShiftX, float fShiftZ)
{
	...
	for ( int i = 0; i < (m_nWidthDimInGrids+1)*(m_nDepthDimInGrids+1); ++i )
	{
		sc::CWireBox * pWireBox = CSceneManager::GetInstance()->AddNodeWireBox(fHeight,fWidth,fDepth,0xffff0000,false,NULL);
...
D3DXVECTOR3 const & CModel::GetOffset()
{
	return m_vOffset;
}

void CModel::SetHeightBoxesVisibility(bool bIsVisible)
{
...
void CModel::InitRotation()
{
	D3DXMatrixIdentity(&m_matRotY);
	m_fRotation = 0.0f;
}

void CModel::UpdateRotation()
{
	m_fRotation += 0.1f;
	D3DXMatrixRotationY(&m_matRotY,m_fRotation);
}

void CModel::InitLift()
{
	m_fLift = 0.0f;
	this->UpdateBoundingBox();
}

void CModel::InitShiftX()
{
	m_fShiftX = 0.0f;
	this->UpdateBoundingBox();
}

void CModel::InitShiftZ()
{
	m_fShiftZ = 0.0f;
	this->UpdateBoundingBox();
}

void CModel::UpdateLift(int nWay)
{
	if ( nWay == 0 )
		m_fLift -= 4.0f;
	if ( nWay == 1 )
...
void CModel::UpdateShiftX(int nWay)
{
	if ( nWay == 0 )
		m_fShiftX -= 4.0f;
	if ( nWay == 1 )
...
void CModel::UpdateShiftZ(int nWay)
{
	if ( nWay == 0 )
		m_fShiftZ -= 4.0f;
	if ( nWay == 1 )
...
float CModel::GetLift()
{
	return m_fLift;
}

float CModel::GetShiftX()
{
	return m_fShiftX;
}

float CModel::GetShiftZ()
{
	return m_fShiftZ;
}
...
void CModel::RestoreLift(float f)
{
	m_fLift = f;
}

void CModel::RestoreRotation(float f)
{
	m_fRotation = f;
	D3DXMatrixRotationY(&m_matRotY,m_fRotation);
}
...
void CModel::SetOffsetOverall(D3DXVECTOR3 vec)
{
...
void CModel::SetCenterGridIndex(int n)
{
	...
	if ( n != INVALID )
	{
		if ( !this->IsSubUnit() )
		{
			CTerrainMesh::SGrid & grid = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(n);
...
void CModel::RestoreCenter(D3DXVECTOR3 & v)
{
	m_vCenter = v;
...
int CModel::GetOriginAdjustedCenterGridIndex(int n)
{
	CTerrainMesh::SGrid & grid = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(n);
	int nDeltaX = 0;
	int nDeltaZ = 0;
	...
	return n + nDeltaX + nDeltaZ*CXFile::GetInstance()->GetTerrainMesh()->GetWidth();
}
...
//	{
//		CTerrainMesh::SVertex & vertex = CXFile::GetInstance()->GetTerrainMesh()->GetVertex(*iter);
...
//		pos = vCenter + dir;
//		
//		int n = (int)pos.x/GRID_SPACE + (int)pos.z/GRID_SPACE * CXFile::GetInstance()->GetTerrainMesh()->GetWidth();
//		
//		CTerrainMesh::SGrid & grid = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(n);
//		
//		int nCnt = 0;
...
//		D3DXVECTOR3 v[4];
//		
//		v[0] = CXFile::GetInstance()->GetTerrainMesh()->GetVertex(grid.dwVertexIndex[0]).vPosition;
//		v[1] = CXFile::GetInstance()->GetTerrainMesh()->GetVertex(grid.dwVertexIndex[1]).vPosition;
//		v[2] = CXFile::GetInstance()->GetTerrainMesh()->GetVertex(grid.dwVertexIndex[2]).vPosition;
//		v[3] = CXFile::GetInstance()->GetTerrainMesh()->GetVertex(grid.dwVertexIndex[3]).vPosition;
//		
...
//
//		// up
//		CTerrainMesh::SGrid & gridUp = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(*iter+CXFile::GetInstance()->GetTerrainMesh()->GetWidth());
//		for ( int i = 0; i< 4; i++)
//		{
...
//
//		// down
//		CTerrainMesh::SGrid & gridDown = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(*iter-CXFile::GetInstance()->GetTerrainMesh()->GetWidth());
//		for ( int i = 0; i< 4; i++)
//		{
...
//
//		// left
//		CTerrainMesh::SGrid & gridLeft = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(*iter-1);
//		for ( int i = 0; i< 4; i++)
//		{
...
//
//		// right
//		CTerrainMesh::SGrid & gridRight = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(*iter+1);
//		for ( int i = 0; i< 4; i++)
//		{
...
//
//		// rightup
//		CTerrainMesh::SGrid & gridRightUp = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(*iter+CXFile::GetInstance()->GetTerrainMesh()->GetWidth()+1);
//		for ( int i = 0; i< 4; i++)
//		{
...
//
//		// leftup
//		CTerrainMesh::SGrid & gridLeftUp = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(*iter+CXFile::GetInstance()->GetTerrainMesh()->GetWidth()-1);
//		for ( int i = 0; i< 4; i++)
//		{
...
//
//		// rightdown
//		CTerrainMesh::SGrid & gridRightDown = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(*iter-CXFile::GetInstance()->GetTerrainMesh()->GetWidth()+1);
//		for ( int i = 0; i< 4; i++)
//		{
...
//
//		// leftdown
//		CTerrainMesh::SGrid & gridLeftDown = CXFile::GetInstance()->GetTerrainMesh()->GetGrid(*iter-CXFile::GetInstance()->GetTerrainMesh()->GetWidth()-1);
//		for ( int i = 0; i< 4; i++)
//		{
...
void CModel::SetBlockType(std::vector<sc::CModel::SBlockInfo> & vecBlock) 
{
...
//void CModel::SetBlockLogicHeight(std::vector<sc::CModel::SBlockInfo> & vecBlock) 
//{
//	for ( int i = 0; i < vecBlock.size(); ++i )
//	{
...
void CModel::SetLogicHeight(std::vector<sc::CModel::SLogicHeightInfo> & vecHeight) 
{
...
void CModel::SetBlockMask(std::vector<sc::CModel::SBlockInfo> & vecBlock) 
{
	for ( int i = 0; i < vecBlock.size(); ++i )
	{
...
void CModel::UpdateHeightBoxes()
{
	...
	{
		D3DXVECTOR3 vCenter = CXFile::GetInstance()->GetTerrainMesh()->GetVertex(m_vecBlockVertexIndices[i]).vPosition;
		m_vecpWireBox[i]->SetRelativePos(vCenter);
		m_vecpWireBox[i]->UpdateAbsolute();
	}
}

void CModel::IncreaseBoxHeightByIndex(int n)
{
	m_vecpWireBox[n]->IncreaseHeight();
	m_vecLogicHeightInfo[n].fLogicHeight = m_vecpWireBox[n]->GetHeight();
}

void CModel::DecreaseBoxHeightByIndex(int n)
{
	m_vecpWireBox[n]->DecreaseHeight();
	m_vecLogicHeightInfo[n].fLogicHeight = m_vecpWireBox[n]->GetHeight();
}

void CModel::ClearBoxHeightByIndex(int n)
{
	m_vecpWireBox[n]->SetHeight(0.0f);
	m_vecLogicHeightInfo[n].fLogicHeight = 0.0f;
}

float CModel::GetBoxHeightByIndex(int n)
{
	return 	m_vecLogicHeightInfo[n].fLogicHeight;
}

void CModel::SetAnimationByAraFileName(std::string strAraFileName)
{
	IAnimationGroup * pAnimationGroup;

	CXFile::GetInstance()->GetOperator()->GetCoreGraphic()->CreateAnimationGroup(strAraFileName.c_str(), &pAnimationGroup);
...
int CModel::GetBlockTypeByIndex(int i) 
{	
	return m_vecBlockInfo[i].nBlockType; 
}

void CModel::LinkEffect(IEffect * pEffect, IEffectGroup * pGroup)
{
	m_pGroup = pGroup;
	m_pEffect = pEffect;	
