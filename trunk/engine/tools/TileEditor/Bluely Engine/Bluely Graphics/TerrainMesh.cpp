...
/*
	...
	CTerrainMesh * pTerrain = sc::CSceneManager::GetInstance()->AddNodeTerrainMesh(dwWidth,dwDepth,dwTextureHandleDefault);
	sc::CTileEditApp::GetInstance()->SetTerrainMesh(pTerrain);
	...
	for ( int i = 0; i< pTerrain->m_dwGridCnt; i++ )
	{
		...
		if ( dw != 0 )
		{
			for ( int i = 0; i < dw; i++ )
			{
				...
				sc::CModel * pModel = sc::CSceneManager::GetInstance()->AddNodeObject(strFileName, bSubUnit);
