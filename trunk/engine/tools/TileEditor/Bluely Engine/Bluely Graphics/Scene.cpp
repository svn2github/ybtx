...
CTileEditorModel * CSceneManager::AddNodeModel(std::string strFileName, bool bSubUnit, float fLift, float fShiftX, float fShiftZ, ISceneNode * pParent)
{
	if ( !pParent )
		pParent = this;
	
	CTileEditorModel * pNode = new CTileEditorModel(pParent);
	pNode->Create(strFileName, bSubUnit, fLift, fShiftX, fShiftZ);
	return pNode;
}
