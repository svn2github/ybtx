...
private:
	...
	{
		return m_dwTextureHandle = bg::rd::CRenderDevice::GetInstance()->CreateTexture(strFilename);
	}
...
public:
	...
	{ 
		...
		if ( m_nCount == 0 ) 
		{ 
			bg::rd::CRenderDevice::GetInstance()->DestroyTexture(m_dwTextureHandle);
	...
	static void CSharedTexture::Clear()
	{
		...
		for ( ; iter != m_map.end(); ++iter )
		{
			...
			{
				bg::rd::CRenderDevice::GetInstance()->DestroyTexture(iter->second->m_dwTextureHandle);
				delete iter->second;
			}
		}
