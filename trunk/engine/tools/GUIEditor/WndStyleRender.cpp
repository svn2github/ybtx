...
void WndStyleRender::SetBackTexture(ITexture* pText)
{
	m_pText = pText;
	if(m_pText)
	{
...
void WndStyleRender::SetDrawRect(const CFRect& Rect)
{
	m_fRect = Rect;
}

void WndStyleRender::SetOffSet( float X, float Y )
{
	m_fOffX += X;
	m_fOffY += Y;
}

void WndStyleRender::BeginRenderer()
{	
...
void WndStyleRender::EndRenderer()
{
	if(CMainWindowTarget::HasInst())
	{
		m_bStateInRender = false;
