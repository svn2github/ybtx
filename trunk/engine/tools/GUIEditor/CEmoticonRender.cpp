...
CTextureFontRender::CTextureFontRender()
: m_fOffX(0.0f)
, m_fOffY(0.0f)
, m_Width(0)
, m_Height(0)
...
void CTextureFontRender::SetBackTexture(ITexture* pText)
{
	m_pText = pText;
	if(m_pText)
	{
...
void CTextureFontRender::SetDrawRect(const CFRect& Rect)
{
	m_fRect = Rect;
}

void CTextureFontRender::SetOffSet( float X, float Y )
{
	m_fOffX += X;
	m_fOffY += Y;
}

void CTextureFontRender::BeginRenderer()
{	
...
void CTextureFontRender::EndRenderer()
{
	if(CMainWindowTarget::HasInst())
	{
		m_bStateInRender = false;
...
void CTextureFontRender::Render()
{
