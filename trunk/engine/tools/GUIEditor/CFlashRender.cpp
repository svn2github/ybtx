...
void CFlashRender::SetBackTexture(ITexture* pText)
{
	m_pText = pText;
	if(m_pText)
	{
...
void CFlashRender::SetDrawRect(const CFRect& Rect)
{
	m_fRect = Rect;
}

void CFlashRender::SetOffSet( float X, float Y )
{
	m_fOffX += X;
	m_fOffY += Y;
}

void CFlashRender::BeginRenderer()
{	
