...
void CMd5Digester::Process(const uint8	*pData)
{
	uint
...
void CMd5Digester::Append(const uint8 *pData, uint	uLength)
{
	const uint8	*p = pData;
	uint left =	uLength;
...
void CMd5Digester::End(uint8 pDigest[16])
{
...
void CMd5Digester::GetMD5Base16Str(char* pBase16)
{
	uint8 pDigest[16];
	End( pDigest );
